...
// declarators, which are scope containers themselves). The container may be
// any function-like node, a variable declarator or property whose
// initializer is function-like, or a namespace declaration; anything else
// yields nil.
func ScopeDeclarations(container *ast.Node) []*ast.Node {
	body := scopeBody(container)
	if body == nil {
		return nil
	}
	var decls []*ast.Node
...
// identifier-named declarators (binding patterns are skipped).
...
// be named (anonymous callbacks, IIFEs, class expressions, static blocks):
// those keep the `@pos` fallback.
...
func namedScopeOf(n *ast.Node) (name string, container *ast.Node, isScope bool) {
	...
	case ast.KindFunctionExpression, ast.KindArrowFunction:
		...
		return "", n, true
	case ast.KindClassExpression, ast.KindClassStaticBlockDeclaration:
		return "", n, true
	}
	return "", nil, false
}
