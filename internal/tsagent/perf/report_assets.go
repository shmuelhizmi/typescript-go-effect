...
// reportCSS is the inline stylesheet for the HTML report.
const reportCSS = `
...
// treemapJS lays out and renders the squarified treemap from the embedded TREE
// data, with a heat scale and hover tooltip. No external dependencies.
const treemapJS = `
(function(){
  var root=document.getElementById('treemap');
  var tt=document.createElement('div'); tt.id='tt'; document.body.appendChild(tt);
  var MINW=48,MINH=26,HDR=15,PAD=2,MAXD=7;
  var maxLeaf=0.0001;
  (function fm(n){ if(!n.ch||!n.ch.length){ if(n.t>maxLeaf)maxLeaf=n.t; } else n.ch.forEach(fm); })(TREE);

  function val(n){ return Math.max(n.t,0.0001); }
  function heat(t){
  ...
  function render(node,x,y,w,h,depth,parent){
    ...
    if(leaf||w<MINW||h<MINH||depth>MAXD){
      var c=el('tm-cell',x,y,w,h); c.style.background=heat(node.t/maxLeaf);
    ...
    if(iw<MINW||ih<MINH){var c2=el('tm-cell',x,y,w,h);c2.style.background=heat(node.t/maxLeaf);c2._node=node;c2._agg=true;
  ...
  function draw(){
    ...
    if(!TREE.ch||!TREE.ch.length){root.innerHTML='<div style="padding:18px;color:#8893a6">No file timing data.</div>';return;}
    ...
    rects.forEach(function(rc){render(rc.n,rc.x,rc.y,rc.w,rc.h,1,root);});
  }

  root.addEventListener('mousemove',function(e){
    var t=e.target; while(t&&t!==root&&!t._node)t=t.parentNode;
    if(!t||!t._node){tt.style.display='none';return;}
    var n=t._node;
    var name=n.path||n.n;
    var extra=t._agg?'<div class="row" style="color:#8893a6;margin-top:4px">folder aggregate</div>':'';
    tt.innerHTML='<div class="p">'+name+'</div>'+
      '<div class="row"><span>total</span><b>'+fmt(n.t)+' ms</b></div>'+
      '<div class="row"><span>check</span><b>'+fmt(n.c)+' ms</b></div>'+
      '<div class="row"><span>parse / bind</span><b>'+fmt(n.p)+' / '+fmt(n.b)+' ms</b></div>'+
      '<div class="row"><span>types</span><b>'+n.y+'</b></div>'+extra;
  ...
  root.addEventListener('mouseleave',function(){tt.style.display='none';});
  var rt;addEventListener('resize',function(){clearTimeout(rt);rt=setTimeout(draw,120);});
  draw();
})();
`

// navJS highlights the sidebar link for whichever section is currently in view.
const navJS = `
(function(){
